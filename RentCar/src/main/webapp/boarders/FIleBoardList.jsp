...
<%
	...
	List<FileBoardVO> list = (ArrayList)request.getAttribute("list");
...
<%		 
		...
		for(int cnt=beginPerPage;   cnt<(beginPerPage+numPerPage);   cnt++){
			...
			FileBoardVO vo  = (FileBoardVO)list.get(cnt);
...
<tr>
	...
	%>		
		<input type="image"
			   ...
			   onclick="location.href='<%=contextPath%>/FileBoard/write.bo?nowBlock=<%=nowBlock%>&nowPage=<%=nowPage%>'"
...
<form action="<%=contextPath%>/FileBoard/seachlist.bo" 
      name="search" 
      method="post"
	...
	<a href="<%=contextPath%>/FileBoard/list.bo?nowBlock=0&nowPage=0">[처음으로]</a>
