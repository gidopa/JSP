...
public class MemberDAO {
	...
	//LoginServlet에서 호출할 회원정보가 DB에 있는지 확인하는 메소드
	public boolean isExisted(MemberVO vo) {
		...
		try {
			con = dataSource.getConnection();
			// 오라클 decode(count(*),1,'true','false') 함수
			// 레코드행이 1개가 조회되면 true반환 아니면 false 반환
			...
			rs = pstmt.executeQuery();
			rs.next(); // 커서를 조회된 첫번째 레코드행으로 위치시킴
			result = Boolean.parseBoolean(rs.getString("result"));
		}catch(Exception e) {
			System.out.println("쿼리문 오류" + e);
		}
		return result;
	}
